...
class AllProfilesData:
    def __init__(self):
        self.title = ""
        self.water_depth = 250


class FilteredProfilesSlot(BaseView):
    _width = 300
    _height = 400
    ...
    def _parameter_selected(self, event: MenuItemClick):
        if event.item == self._parameter:
            return
        self._parameter = event.item
        self._parameter_dropdown.label = expand_abbreviation(self._parameter)
        self._filtered_profiles_model.selected_profile_index = self._profile_index
    ...
    def set_data(
        ...
        parameter_name, parameter_data = data

        if parameter_data is None:
            return

        self._source.data = parameter_data
        self._values.name = expand_abbreviation(parameter_name)
        self._parameter_data = parameter_data["data"]
    ...
    def _sync_y_axis(self):
        # set yaxis range
        y_min = -5
        y_max = (
            max(
                [
                    getattr(self._visit, "water_depth", 0),
                    getattr(self._visit, "max_depth", 0),
                ]
            )
            + 5
        )
    ...
    def select_values(self, rows):
        ...
        if not self._clear_called:
            self._manual_qc_model.set_selected_values(selected_values, self)
    ...
    @property
    def profile_index(self) -> int | None:
        return self._profile_index

    @property
    def layout(self):
        return column(self._parameter_dropdown, self._figure)
