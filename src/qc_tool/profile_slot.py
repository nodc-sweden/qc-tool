...
class ProfileSlotBase(Layoutable):
    ...
    def __init__(self, value_selected_callback=None):
    ...
    def update_stations(self, stations: list):
        ...
        # - If one station keep selection if existing or update to first available
        if len(self._stations) > 1 and len(self._parameter_multichoice.value) > 1:
